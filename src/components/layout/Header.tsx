    ...
    if (pathname === '/') {
      ...
      const lenis = (
        window as unknown as {
          __lenis?: { scrollTo: (target: string | number, opts?: unknown) => void };
        }
      ).__lenis;
      ...
      if (href.startsWith('/#')) {
        ...
        if (lenis) {
          lenis.scrollTo(targetId, { offset: -80, duration: 1.1 });
        } else {
      ...
      <header className="sticky top-0 z-40 w-full border-b border-[#e5e7eb] bg-white/90 backdrop-blur-md transition-colors dark:border-[#242831] dark:bg-[#0a0b0d]/90">
        <div className="flex h-18 w-full items-center justify-between px-4 sm:px-6 md:px-8 lg:px-10">
              ...
              return (
                <Link
                  ...
                  className={`text-sm transition-colors ${
                    isActive
                      ? 'font-semibold text-[#121417] dark:text-white'
                      : 'font-medium text-[#4b5563] hover:text-[#121417] dark:text-[#9aa1ad] dark:hover:text-white'
                  }`}
                >
                  {link.label}
                </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Gatilho Mobile: Ícone/Avatar do Perfil abre a navegação mobile */}
            <button
              type="button"
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              aria-label={mobileMenuOpen ? 'Fechar menu de navegação' : 'Abrir menu de navegação e perfil'}
              aria-expanded={mobileMenuOpen}
              className={`flex h-10 w-10 items-center justify-center overflow-hidden rounded-full border transition-all md:hidden ${
                mobileMenuOpen
                  ? 'border-[#121417] bg-[#121417] text-white dark:border-white dark:bg-white dark:text-[#121417]'
                  : 'border-[#e5e7eb] bg-[#f8f9fa] text-[#121417] hover:border-[#121417] dark:border-[#242831] dark:bg-[#181b22] dark:text-[#f3f4f6]'
              }`}
            >
              {mobileMenuOpen ? (
                <X className="h-5 w-5" />
                ...
                return (
                  <Link
                    ...
                    className={`text-base transition-colors ${
                      isActive
                        ? 'font-bold text-[#121417] dark:text-white'
                        : 'font-semibold text-[#64748b] hover:text-[#121417] dark:text-[#9aa1ad] dark:hover:text-white'
                    }`}
                  >
                    {link.label}
                  </Link>
